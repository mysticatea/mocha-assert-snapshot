...
var fs = require('fs');
var format = require('pretty-format');
...
function isDirectory(filePath) {
    return new Promise(resolve => {
        fs__default['default'].stat(filePath, (_error, stats) => {
...
function mkdir(filePath) {
    return new Promise((resolve, reject) => {
        fs__default['default'].mkdir(filePath, error => {
...
class Snapshot {
    ...
    assert(key, value) {
        ...
        const actual = format__default['default'](value);
    ...
    load() {
        return new Promise((resolve, reject) => {
            fs__default['default'].readFile(this.filePath, "utf8", (readError, data) => {
                if (readError != null) {
                    //istanbul ignore else
    ...
    save() {
        return new Promise((resolve, reject) => {
            ...
            ensureDirectory(dirname(this.filePath)).then(() => {
                fs__default['default'].writeFile(this.filePath, data, writeError => {
                    //istanbul ignore else
                    if (writeError == null) {
                        resolve(this);
                    }
...
exports.assertSnapshotThrows = assertSnapshotThrows;
