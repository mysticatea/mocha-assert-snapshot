...
describe("Run on multiple files (parallel)", () => {
    ...
    describe("mocha --parallel --require mocha-assert-snapshot test/*.js", () => {
    ...
    describe("MOCHA_ASSERT_SNAPSHOT=update mocha --require mocha-assert-snapshot --update test/*.js", () => {
        let result: Executor.Result
        before(async () => {
            result = await executor.mochaWithEnv(
                ["--parallel", "--require", RegisterPath, "--", "test/*.js"],
